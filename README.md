...
| <a name="input_peering_attachment_accepters"></a> [peering\_attachment\_accepters](#input\_peering\_attachment\_accepters) | Map of peering attachments created by another transit gateway to accept in this one | <pre>map(object({<br/>    transit_gateway_attachment_id = string<br/>    tags                          = optional(map(string), {})<br/>  }))</pre> | `{}` | no |
| <a name="input_peering_attachments"></a> [peering\_attachments](#input\_peering\_attachments) | Map of peering attachments to create from this transit gateway to one in another Region or account. The other side must accept them | <pre>map(object({<br/>    peer_transit_gateway_id = string<br/>    peer_region             = string<br/>    # Defaults to the account the provider is connected to<br/>    peer_account_id = optional(string)<br/>    # Used when `create_tgw` is `false` and the transit gateway is managed elsewhere<br/>    transit_gateway_id = optional(string)<br/>    options = optional(object({<br/>      dynamic_routing = optional(string)<br/>    }))<br/>    tags = optional(map(string), {})<br/>  }))</pre> | `{}` | no |
| <a name="input_prefix_list_references"></a> [prefix\_list\_references](#input\_prefix\_list\_references) | Map of prefix list references to create in a transit gateway route table. One reference stands in for every CIDR in the prefix list, which keeps a large network inside the routes per route table quota | <pre>map(object({<br/>    prefix_list_id = string<br/>    # Defaults to the route table this module creates<br/>    transit_gateway_route_table_id = optional(string)<br/>    # Omit to blackhole traffic matching the prefix list<br/>    transit_gateway_attachment_id = optional(string)<br/>    blackhole                     = optional(bool)<br/>  }))</pre> | `{}` | no |
...
| <a name="input_tgw_peering_attachment_tags"></a> [tgw\_peering\_attachment\_tags](#input\_tgw\_peering\_attachment\_tags) | Additional tags for the peering attachments | `map(string)` | `{}` | no |
...
| <a name="input_vpc_attachment_accepters"></a> [vpc\_attachment\_accepters](#input\_vpc\_attachment\_accepters) | Map of VPC attachments created in another account to accept in this one. A transit gateway exists to connect accounts, and an attachment made elsewhere is not usable until it is accepted here | <pre>map(object({<br/>    transit_gateway_attachment_id = string<br/>    # Both default to `true` in the API. Set to `false` when the attachment should use a<br/>    # route table other than the transit gateway's defaults<br/>    transit_gateway_default_route_table_association = optional(bool)<br/>    transit_gateway_default_route_table_propagation = optional(bool)<br/>    tags                                            = optional(map(string), {})<br/>  }))</pre> | `{}` | no |
...
| <a name="output_ec2_transit_gateway_peering_attachment_accepter_ids"></a> [ec2\_transit\_gateway\_peering\_attachment\_accepter\_ids](#output\_ec2\_transit\_gateway\_peering\_attachment\_accepter\_ids) | Map of peering attachment identifiers accepted by this transit gateway |
| <a name="output_ec2_transit_gateway_peering_attachment_ids"></a> [ec2\_transit\_gateway\_peering\_attachment\_ids](#output\_ec2\_transit\_gateway\_peering\_attachment\_ids) | Map of peering attachment identifiers created by this transit gateway |
| <a name="output_ec2_transit_gateway_prefix_list_reference_ids"></a> [ec2\_transit\_gateway\_prefix\_list\_reference\_ids](#output\_ec2\_transit\_gateway\_prefix\_list\_reference\_ids) | Map of prefix list reference identifiers |
...
| <a name="output_ec2_transit_gateway_vpc_attachment_accepter_ids"></a> [ec2\_transit\_gateway\_vpc\_attachment\_accepter\_ids](#output\_ec2\_transit\_gateway\_vpc\_attachment\_accepter\_ids) | Map of accepted VPC attachment identifiers |
